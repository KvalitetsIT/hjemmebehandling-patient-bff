...
public class QuestionnaireResponseServiceTest {
    ...
    public void getQuestionnaireResponses_responsesPresent_returnsResponses() throws Exception {
        ...
        QuestionnaireResponse response = buildQuestionnaireResponse();
    ...
    /**
     * This test is supposed to check that the service is outputting the correct entities and in the correct order following the specified pagination
     */
    @Test
    public void getQuestionnaireResponsesForMultipleCarePlans_ReturnSortedPages() throws Exception {
        QuestionnaireResponseService subject = new QuestionnaireResponseService(fhirClient, fhirMapper, null, triageEvaluator, accessValidator);

        List<QuestionnaireResponse> responses = new LinkedList<>();

        List<String> carePlanIds = List.of("carePlan-1", "carePlan-2");
        List<String> questionnaireIds = List.of("questionnaire-1", "questionnaire-2");

        final int months = 12;

        for (int i = 0; i < months; i++) {
            QuestionnaireResponse response = new QuestionnaireResponse();
            response.setAuthored(new Date(i, i, i));
        ...
        FhirLookupResult result = FhirLookupResult.fromResources(responses.toArray(new Resource[0]));
        result.merge(FhirLookupResult.fromResources(patient));


        Mockito.when(fhirClient.lookupQuestionnaireResponsesForMultipleCarePlans(carePlanIds, questionnaireIds)).thenReturn(result);

        for (QuestionnaireResponse respons : responses) {
            Mockito.when(fhirMapper.mapQuestionnaireResponse(respons, result, List.of())).thenReturn(questionnaireResponseToQuestionnaireResponseModel(respons));
        }

        List<List<QuestionnaireResponseModel>> results = new LinkedList<>();

        final int pageSize = 2;

        for (int i = 0; i < (months / pageSize); i++) {
            PageDetails pageDetails1 = new PageDetails(i + 1, pageSize);
            results.add(subject.getQuestionnaireResponsesForMultipleCarePlans(carePlanIds, questionnaireIds, pageDetails1));
        }

        for (int i = 0; i < results.size(); i++) {
            List<QuestionnaireResponseModel> r = results.get(i);
            assertEquals(pageSize, r.size());
            for (int j = 0; j < pageSize; j++) {
                var id = (responses.size() - 1 - (j + i * 2));
                assertEquals(Integer.toString(id), r.get(j).getId().getId());
            }
    ...
    public void getQuestionnaireResponses_ReturnSortedPages_WhenTwoPages() throws Exception {
        ...
        assertEquals("4", result1.getFirst().getId().getId());
        assertEquals("3", result1.get(1).getId().getId());
        assertEquals("2", result2.getFirst().getId().getId());
    ...
    public void submitQuestionnaireResponse_success_populatesAttributes() throws Exception {
        ...
        subject.submitQuestionnaireResponse(questionnaireResponseModel, cpr);
    ...
    public void submitQuestionnaireResponse_success_computesTriagingCategory() throws Exception {
        ...
        Mockito.when(triageEvaluator.determineTriagingCategory(
                questionnaireResponseModel.getQuestionAnswerPairs().stream().map(QuestionAnswerPairModel::getAnswer).collect(Collectors.toList()),
                carePlanModel.getQuestionnaires().getFirst().getThresholds())
    ...
    public void submitQuestionnaireResponse_submittedInTime_refreshesSatisfiedUntil() throws Exception {
        ...
        CarePlanModel carePlanModel = buildCarePlanModel(POINT_IN_TIME);
        Mockito.when(fhirMapper.mapCarePlan(carePlan, lookupResult)).thenReturn(carePlanModel);
        ...
        var questionnaireWrapper = carePlanModel.getQuestionnaires().getFirst();
    ...
    public void submitQuestionnaireResponse_refreshesSatisfiedUntilForCarePlan() throws Exception {
        ...
        FhirLookupResult lookupResult = FhirLookupResult.fromResources(carePlan);
        Mockito.when(fhirClient.lookupActiveCarePlans(cpr)).thenReturn(lookupResult);
        CarePlanModel carePlanModel = buildCarePlanModel(List.of(
    ...
    private CarePlanModel buildCarePlanModel() {
        ...
        return buildCarePlanModel(satisfiedUntil);
    }

    private CarePlanModel buildCarePlanModel(Instant satisfiedUntil) {
        ...
        return buildCarePlanModel(List.of(questionnaireWrapper));
    }

    private CarePlanModel buildCarePlanModel(List<QuestionnaireWrapperModel> questionnaires) {
        CarePlanModel carePlanModel = new CarePlanModel();
        carePlanModel.setId(new QualifiedId(QuestionnaireResponseServiceTest.CAREPLAN_ID_1));
        carePlanModel.setPatient(new PatientModel());
        carePlanModel.getPatient().setId(new QualifiedId(QuestionnaireResponseServiceTest.PATIENT_ID));
    ...
    private Patient buildPatient() {
        Patient patient = new Patient();
        patient.setId(QuestionnaireResponseServiceTest.PATIENT_ID);
        return patient;
    }
    ...
    private QuestionnaireResponse buildQuestionnaireResponse() {
        QuestionnaireResponse response = new QuestionnaireResponse();
        response.setId(QuestionnaireResponseServiceTest.QUESTIONNAIRE_RESPONSE_ID_1);
        response.setQuestionnaire(QuestionnaireResponseServiceTest.QUESTIONNAIRE_ID_1);
        response.setSubject(new Reference(QuestionnaireResponseServiceTest.PATIENT_ID));
        response.addExtension(Systems.ORGANIZATION, new Reference(ORGANIZATION_ID_1));
        return response;
    }
